...
- **Web player** — play collections, series, favorite lists, and single videos directly in the browser
- **Audio-only mode** — hides video, shows cover art; works with iOS lock screen controls and next/previous track
...
- **Merged stream** — ffmpeg muxes separate Bilibili DASH video+audio tracks into a single stream for external players
...
| `video=0` | Audio-only tracks (default: `1` = video+audio) |
| `redirect=1` | Redirect to Bilibili CDN directly — faster for mobile apps (default: `0` = proxy) |
...
|---|---|---|
| Stream route | Browser → your server → Bilibili CDN | Browser → your server (resolve) → 302 → Bilibili CDN |
...
|---|---|
| `GET /api/stream/merged/{bvid}?page=0` | Video + audio muxed via ffmpeg (for external players) |
| `GET /api/stream/video/{bvid}?page=0` | Video track only |
| `GET /api/stream/audio/{bvid}?page=0` | Audio track only |
